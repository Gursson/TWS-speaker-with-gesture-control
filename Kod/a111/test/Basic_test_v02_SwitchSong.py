...
def main():
    ...
    while not interrupt_handler.got_signal:#for i in range(lengd):
        ...
        print(index)
        # Swipe Menu Start 
        if index[1] > 70 and Swipe_timer_1 == 0 and Menu_timer == 0 and (time.time() - Timer_Control) > 6: #Timer, not spamming when holding hand
        ...
        if Menu_timer > 0 :
           # print(time.time() - Menu_timer) 
            ...
            elif index[1] > 70 and Swipe_timer_1 == 0 and Timer_H2 == 0:# and index[0] < 120:
                Counter_H1 = Counter_H1 + 1
                if Counter_H1 > DataPoint_Swipe:
                    Timer_H1 = time.time()
            else:
                Counter_H1 = 0     
            if 0 < (time.time() - Timer_H1) < 0.5 and index[0] > 70 :
            ...
            if index[0] > 70 and Swipe_timer_0 == 0 and Timer_H1 == 0:# and index[1] < 120:
                Counter_H2 = Counter_H2 + 1
                if Counter_H2 > DataPoint_Swipe:
                    Timer_H2 = time.time()
            else:
                Counter_H2 = 0     
            if 0 < (time.time() - Timer_H2) < 0.5 and index[1] > 70 :
